...
export async function generateEmail(payload: EmailGenerateRequest) {
  return request<EmailGenerateResponse>(`/api/email/generate`, {
    method: 'POST',
...
export async function getUserProfile(userId: string) {
  return request<UserProfile>(`/api/users/${encodeURIComponent(userId)}/profile`)
}
...
export async function getDraftHistory(userId: string, limit = 20) {
  ...
  return request<DraftHistoryResponse>(
    `/api/users/${encodeURIComponent(userId)}/history?${params.toString()}`,
  )
}

export async function learnFromEdits(userId: string, payload: LearnFromEditsPayload) {
  return request<{ status: string }>(
    `/api/users/${encodeURIComponent(userId)}/history/learn`,
    {
      method: 'POST',
      body: JSON.stringify(payload),
    },
